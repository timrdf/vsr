...
while [ $# -gt 0 ]; do
   ...
   for object in `o-of-p.sh $follow $outfile | sort -u | grep 'http://ieeevis.tw.rpi.edu/' | grep -v "–"`; do
   ...
   for sameas in $sames; do
      ...
      for object in `o-of-p.sh --inverse-of $sameas $outfile | sort -u | grep 'http://ieeevis.tw.rpi.edu/' | grep -v "–"`; do
         if [[ `grep "^$object$" $visited` ]]; then
            echo "`void-triples.sh $outfile | sed 's/./ /g'` | $object" >&2
         else
            rapper -q -g -o turtle $object >> $outfile
